...
- Confirm the deploy role in each environment has `ssm:GetParameter` on those paths.
...
- Repo: `https://github.com/productsweet-bootstrap-e2e-app/ai6p-2068-external-webapp`
...
3. Confirm each environment's deploy role grants `ssm:GetParameter` on the prefix (a
   parameter that exists but is unreadable fails the build identically).
